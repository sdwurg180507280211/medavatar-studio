...
const Shell: React.FC<{title: string; children: React.ReactNode}> = ({title, children}) => (
  <div style={{position:'absolute', left:70, top:70, width:1360, height:790, borderRadius:36, background:'#F7FBFD', overflow:'hidden', boxShadow:'0 30px 90px rgba(0,0,0,.22)'}}>
      ...
      <div style={{position:'absolute', left:170, right:170, top:300, height:210, borderRadius:130, background:colors.vessel, transform:`scaleY(${beat})`, transformOrigin:'center'}}>
        <div style={{position:'absolute', inset:42, borderRadius:100, background:colors.lumen, overflow:'hidden'}}>
          {Array.from({length:7}).map((_, index) => (
            <div key={index} style={{position:'absolute', width:42, height:42, borderRadius:'50%', background:'#FFF', top:26 + (index % 2) * 54, left:((index * 150 + flow) % 1040) - 80, boxShadow:'0 3px 12px rgba(42,116,255,.16)'}} />
          ))}
        </div>
      </div>
      {['↑','↓'].map((symbol, index) => (
        <div key={symbol} style={{position:'absolute', left:650, top:index === 0 ? 205 : 535, fontSize:78, lineHeight:1, fontWeight:900, color:colors.red, opacity:arrow, transform:`translateY(${interpolate(arrow,[0,1],[index === 0 ? 20 : -20,0])}px)`}}>{symbol}</div>
      ))}
  ...
  return (
    <Shell title="内皮受损后，脂质沉积如何逐渐形成斑块">
      <div style={{position:'absolute', left:170, right:170, top:290, height:220, borderRadius:130, background:colors.vessel}}>
        <div style={{position:'absolute', inset:38, borderRadius:100, background:colors.lumen, overflow:'hidden'}}>
          {Array.from({length:6}).map((_, index) => (
            <div key={index} style={{position:'absolute', width:38, height:38, borderRadius:'50%', background:'#FFF', top:34 + (index % 2) * 54, left:((index * 170 + flow) % 960) - 70}} />
          ))}
        </div>
      ...
      <div style={{position:'absolute', left:190, top:590, display:'flex', gap:28, alignItems:'center'}}>
        {['内皮损伤','炎症反应','脂质沉积','管腔变窄'].map((label, index) => (
  ...
  return (
    <Shell title="心脏在持续压力负荷下反复做功">
      ...
      <div style={{position:'absolute', left:700, top:260, width:510, height:220, borderRadius:28, background:'#0E2A3E', overflow:'hidden'}}>
        <svg viewBox="0 0 510 220" width="510" height="220">
          <polyline points="0,125 70,125 98,125 122,90 145,162 174,42 205,125 270,125 312,125 340,88 362,155 390,52 420,125 510,125" fill="none" stroke="#63E5E7" strokeWidth="7" strokeLinecap="round" strokeLinejoin="round" strokeDasharray="1000" strokeDashoffset={1000 * (1 - lineProgress)} />
        </svg>
      </div>
...
const Fallback: React.FC<{scene: Scene}> = ({scene}) => (
  <Shell title={`医学机制动画 · ${scene.animation?.name ?? 'mechanism'}`}>
...
export const MedicalAnimationScene: React.FC<{scene: Scene}> = ({scene}) => {
  switch (scene.animation?.name) {
    case 'artery-pressure': return <ArteryPressure />;
    case 'plaque-growth': return <PlaqueGrowth />;
    case 'heart-beat': return <HeartBeat />;
    case 'risk-pathway': return <RiskPathway />;
